...
pub fn set_camera_rotation(new_rot: [f32; 2]) {
    let process_start = unsafe { GetModuleHandleA(PCSTR::null()).unwrap().0 };
    unsafe { std::ptr::write((process_start + 0x1020C60) as *mut _, new_rot) };
}
...
pub fn get_is_loading() -> bool {
    let process_start = unsafe { GetModuleHandleA(PCSTR::null()).unwrap().0 };
    let loading: bool = unsafe { std::ptr::read((process_start + 0x14005F4) as *const _) };
    return loading;
}

pub fn get_is_cutscene_playing() -> bool {
    let process_start = unsafe { GetModuleHandleA(PCSTR::null()).unwrap().0 };
    let cutscene_playing: bool = unsafe { std::ptr::read((process_start + 0x102A244) as *const _) };
